...
class Housekeeper():
    ...
    def Birthday(self):
        ...
        if seconds_until > 0:
            return '猪宝，这是小可爱在你26岁生日的时候，送给你的一份生日礼物哦~\n\n还有%s 就是猪宝的生日啦！倒计时%d秒！' % (
                str(datetime.timedelta(seconds=seconds_until)), seconds_until)
        elif -86400 < seconds_until <= 0:
            return '猪宝，生日快乐！小可爱已经准备好啦，正在乖乖就位等待猪宝的疯狂亲亲哦！和猪宝度过的每一天都是值得纪念的一天！'
        elif seconds_until < -86400:
            return '猪宝，这是小可爱在你26岁生日的时候，送给你的一份生日礼物哦~\n\n还有{time} 就是猪宝的{years}岁生日啦！倒计时{seconds_next}秒！' .format(
                time=str(datetime.timedelta(seconds=seconds_next)), years=years, seconds_next=int(seconds_next))

    @robot.filter('天气')
    def weather(self):
        location_id_wzz = '630103'
        path_live = 'http://restapi.amap.com/v3/weather/weatherInfo?extensions=base&key=cab3be98debe655e5249101b8c1eb7d2&city='
        url_live = path_live + location_id_wzz
        response_live = requests.get(url_live)
        result_live = response_live.json()

        location = result_live['lives'][0]['province'] + \
            '省' + result_live['lives'][0]['city']
        ...
        path_fore = 'http://restapi.amap.com/v3/weather/weatherInfo?extensions=all&key=cab3be98debe655e5249101b8c1eb7d2&city='
        ...
        path_dzx = 'http://restapi.amap.com/v3/weather/weatherInfo?extensions=base&key=cab3be98debe655e5249101b8c1eb7d2&city='
    ...
    def chatbot(self):
        msg = self.content
        reply_mes = requests.get('http://api.ruyi.ai/v1/message?app_key=9bedfee0-df4d-459e-854e-e9a9cc2fa24b&user_id=123456&q=' + msg)
        js = reply_mes.text
        js = json.loads(js)
        js = js['result']['intents']
        js = js[0]['outputs'][0]['property']['text']
        return js
